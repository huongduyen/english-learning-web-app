...
import { ListeningLessonCard } from '../components/listening/ListeningLessonCard';
...
export const ListeningListPage: React.FC = () => {
  ...
  const {
    ...
    queryKey: ['listening-lessons', searchTerm, selectedDifficulty, selectedLevel],
    queryFn: () =>
      ...
      <main className="container mx-auto max-w-5xl flex-1 px-4 py-8 sm:px-6">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/10 px-3 py-0.5 text-xs font-semibold text-primary">
              ...
              Module 8 &middot; Listening
            </div>
            <h1 className="mt-2 text-2xl font-extrabold tracking-tight text-foreground sm:text-3xl">
              Listening Comprehension
            </h1>
            <p className="mt-1 text-xs text-muted-foreground sm:text-sm">
              Immerse yourself in authentic English dialogues, train your ear, and test your understanding.
            </p>
          </div>
        </div>
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            <select
              id="listening-difficulty-filter"
        ...
        <div className="mt-8">
          {isLoading && (
            <div id="listening-loading-skeleton" className="grid gap-5 sm:grid-cols-2">
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
          ...
          {isError && (
            ...
            >
              ...
              <h3 className="mt-3 text-base font-bold text-foreground">
                Unable to load listening lessons
              </h3>
              <p className="mt-1 text-xs text-muted-foreground">
                An error occurred while connecting to the audio lesson service.
              </p>
              <button
                type="button"
                id="listening-retry-button"
                onClick={() => refetch()}
          ...
          {!isLoading && !isError && lessons.length === 0 && (
            ...
            >
              ...
              <h3 className="mt-3 text-base font-bold text-foreground">
                No listening lessons available yet.
              </h3>
              <p className="mt-1 text-xs text-muted-foreground max-w-sm">
                Try adjusting your search query or filters to discover available lessons.
              </p>
